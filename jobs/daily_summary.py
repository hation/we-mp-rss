...
def _ai_summarize(text: str, title: str = "") -> str:
    ...
    # 文本过长时截断，避免token超限
    content = text[:4000]
    prompt = (f"请用300-400字详细概括下面这篇公众号文章，"
              f"提炼核心观点、关键数据和重要结论，"
              f"确保内容完整、逻辑清晰、语句通顺，"
              f"以自然段落形式输出，不要中途截断，不要使用序号。\n"
              f"标题：{title}\n正文：{content}")
...
def build_daily_summary() -> str:
    ...
    try:
        ...
        if need_ai:
            ...
            print_info(f"共 {len(items)} 篇，其中 {len(need_ai)} 篇需要新生成AI摘要")
            ...
            for i, idx in enumerate(need_ai):
                ...
                try:
                    ...
                    logger.warning(f"写入AI摘要缓存失败 [{items[idx]['title']}]: {e}")
                    session.rollback()
        else:
            print_info(f"共 {len(items)} 篇，全部命中缓存，跳过AI调用")
...
def start_daily_summary():
    ...
    # 确保调度器已启动（start_job可能因无定时采集任务而跳过启动）
